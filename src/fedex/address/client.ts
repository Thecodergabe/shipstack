...
export function createFedexAddressClient() {
  ...
  try {
    log.debug("Creating FedEx Address client...");
    OpenAPI.BASE = FEDEX_BASE_URL ?? "https://apis.fedex.com";
    OpenAPI.TOKEN = FEDEX_API_KEY;

    log.info("FedEx Address client initialized.");
    return DefaultService;
  } catch (err) {
    log.error("Failed to initialize FedEx Address client", err);
    throw new ShipstackError("FedEx Address client initialization failed", "fedex", err);
  }
}