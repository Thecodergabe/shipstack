...
export function createFedexTrackingClient() {
  ...
  try {
    log.debug("Creating FedEx Tracking client...");
    OpenAPI.BASE = FEDEX_BASE_URL ?? "https://apis.fedex.com";
    OpenAPI.TOKEN = FEDEX_API_KEY;

    log.info("FedEx Tracking client initialized.");
    return DefaultService;
  } catch (err) {
    log.error("Failed to initialize FedEx Tracking client", err);
    throw new ShipstackError("FedEx Tracking client initialization failed", "fedex", err);
  }
}